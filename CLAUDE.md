...
> 每次新对话开始，先读本文件，再读 `PROGRESS.md`（在仓库根目录）。
...
- **不许改名或删除现有六个 `data` 顶层字段**：`accounts`、`balances`、`records`、`budget`、`recurringBills`、`categoryBudgets`。云端存量数据全靠这些 key，改名 = 老用户数据读不出来。**新增字段允许**，但必须在 `migrateData()` 里给老数据补一个默认值。
- **不要改动或删除 `migrateData()` 已有的迁移逻辑**；为新字段追加一行 backfill（补默认值）是允许且推荐的。
...
我说"记住 / 保留 / 这个做得好 / 以后别忘了"时：把结论按日期追加到 `PROGRESS.md`，涉及红线的同时更新本文件。
