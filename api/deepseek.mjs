...
收入分类只能选：工资/收入、退款、资金流转、其他

退款/退回识别（很重要）：如果描述里出现“退款、退货、已退款、退回、返现、返款、报销、撤销支付、收到退款”等表示钱回到账户的情况，则这是一笔收入：
type = "income"
desc = "退款"
note 保留原始商户或事由（如“滴滴打车退款”）
（注意：这类钱是退回/收到，不是花出去，绝不能记成 expense。哪怕金额旁边写的是负号，只要语义是退款就按 income。）
